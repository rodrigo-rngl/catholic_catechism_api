...
from src.application.services.exceptions.empty_paragraph_localization_exception import (
    EmptyParagraphLocalizationException,
)
from src.application.services.exceptions.topic_text_split_failed_exception import (
...
class CatechismPageContentSplitter:
    ...
    def __split_topic_text(cls, text: str) -> List:
        ...
        raise TopicTextSplitFailedException(
            f'Não foi possível dividir o texto do tópico: {text}'
        )
    ...
    def __create_payloads(self) -> None:
        if self.__buffer_localization_is_empty():
            raise EmptyParagraphLocalizationException(
