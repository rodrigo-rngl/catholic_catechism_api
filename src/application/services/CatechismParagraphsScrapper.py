...
from src.application.services.CatechismPageContentSplitter import CatechismPageContentSplitter
...
class CatechismParagraphsScrapper:
    ...
    def scrape(self) -> List[Payload]:
        logger.info(
            "CatechismScrapper: Iniciando a raspagem dos parágrafos do catecismo da Igreja Católica..."
        )
        page_links = self.__scrape_main_page_links()

        for link in page_links:
            logger.info(f"CatechismScrapper: Raspando a página: {link}")
            ...
            logger.info(
                f"CatechismScrapper: {self.catechism_page_content_splitter.n_splitted_paragraphs} parágrafos do Catecismo foram extraídos e estruturados com sucesso. {len(self.catechism_page_content_splitter.payloads_list)} parágrafos no total."
            )

        logger.info(
            "CatechismScrapper: Raspagem dos parágrafos do catecismo da Igreja Católica concluída com sucesso!"
        )
        return self.catechism_page_content_splitter.payloads_list

    def __scrape_main_page_links(self) -> List[str]:
        logger.info(
            "CatechismScrapper: Raspando os links da página principal do site do catecismo da Igreja Católica..."
        )
    ...
    def __page_request(self, page_link: str) -> requests.Response:
        try:
            ...
            message = f"Exceção ao enviar requisição para {self.main_url + page_link}."
            logger.exception(
                f"CatechismScrapper: {message}",
                exc_info=exception,
            )
            raise CatechismParagraphsScrapperException(message) from exception
    ...
    def __find_content_container(cls, soup: BeautifulSoup) -> Tag:
        ...
        if len(table_tags) < 2:
            raise MainPageTableTagsInsufficientException(
        ...
        if not isinstance(table_tag, Tag):
            raise MainPageTableTagInvalidTypeException(
                "A table_tag deveria ser do tipo Tag."
            )

        tds_tags = table_tag.find_all("td")
        if len(tds_tags) < 2:
            raise MainPageTdTagsInsufficientException(
        ...
        if not isinstance(content_container_tag, Tag):
            raise ContentContainerTagInvalidTypeException(
                "O content_container_tag deveria ser do tipo Tag."
            )

        if len(content_container_tag.find_all("p")) == 0 and len(content_container_tag.find_all("font")) == 0:
            raise ContentTextTagsNotFoundException(
