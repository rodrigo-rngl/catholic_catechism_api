...
from src.application.DTOs.IngestionEmbeddings import IngestionEmbeddingsBase
...
from src.infrastructure.information_retriever_base.vector_db_qdrant.ingestion_point_structures_creators.ingestion_point_structures_creators_factory import IngestionPointStructuresCreatorsFactory

from src.config.logger_config import setup_logger
logger = setup_logger(name="CatechismParagraphsCollectionIngestor")


class CatechismParagraphsCollectionIngestor:
    def __init__(self,
    ...
    async def ingest(self, payloads: List[Payload], batch_size: int) -> None:
        ...
        logger.info(
            f"CatechismParagraphsCollectionIngestor: Iniciando a ingestão de {len(payloads)} payloads em {n_batches} batches..."
        )
    ...
    async def __process_batch(self, payloads: List[Payload]) -> None:
        async with self.__batch_semaphore:
            ...
            logger.info(
                f"CatechismParagraphsCollectionIngestor: Enviando {len(payloads)} parágrafos no {batch_position}º batch..."
            )
            ...
            logger.info(
                f"CatechismParagraphsCollectionIngestor: {len(payloads)} parágrafos do {batch_position}º batch foram transformados com sucesso!"
            )
