...
class HttpRequestSearch(HttpRequestInSearch):
    id: UUID = Field(default_factory=uuid4, description="ID da requisição.")
    created_in: datetime = Field(default_factory=lambda: datetime.now(tz=timezone(
        timedelta(hours=-3))), description="Datetime do momento em que a requisição foi criada.")
...
class HttpRequestValidationError(BaseModel):
    id: UUID = Field(..., description="ID da requisição.")
    created_in: datetime = Field(
        ..., description="Datetime do momento em que a requisição foi criada.")
...
HttpRequestType = TypeVar(
    "HttpRequestType", bound=HttpRequest)
