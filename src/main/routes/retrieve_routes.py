...
from src.application.DTOs.http.HttpResponse import HttpResponseBase
from src.application.DTOs.http.HttpRequest import HttpRequestInRetrieve, HttpRequestRetrieve

from src.interface_adapters.composers.catholic_catechism_paragraphs_retriever_composer import catholic_catechism_paragraphs_retriever_composer
...
async def retrieve_catechism_paragraph(paragraph_number: Annotated[int, Path(ge=1, le=2865)]) -> JSONResponse:
    ...
    try:
        ...
        logger.info(
            f"search_routes: Requisição recebida. ID: {http_request.id}")
